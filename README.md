...
> Production-ready AgentOS starter for Claude Code. Three-tier agent topology (sysadmin / operator / heartbeat) on top of saga-mcp + claude-peers + telegram-mcp. Deployable in one command — installer auto-detects macOS vs. Linux.

## Install

```bash
curl -fsSL https://raw.githubusercontent.com/{REPO_OWNER}/claude-code-template/main/install.sh | bash
```

Or clone first, inspect, then run:

```bash
git clone https://github.com/{REPO_OWNER}/claude-code-template.git
