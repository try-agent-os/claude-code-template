...
#
# agent-type: researcher | outreacher | strategist | (empty = generic)
#   When set, adds --add-dir agents/{agent-type} so agent's CLAUDE.md + SOUL.md auto-load.
# model: claude-sonnet-4-6 (default) | claude-opus-4-6 (for complex/strategic tasks)
#
# Creates tmux session: worker-<task-id>
# Worker dir: logs/workers/<task-id>/  (NOT .claude/ — Claude treats that as sensitive)
# Worker writes result to: logs/workers/<task-id>/result.md
# Worker loops until result.md exists or max iterations/timeout reached
...
TIMEOUT_MIN="${4:-30}"
AGENT_TYPE="${5:-}"
MODEL="${6:-${DEFAULT_MODEL:-claude-sonnet-4-6}}"
HOME="${HOME}"
...
CLAUDE="$5"
AGENT_TYPE="${6:-}"
MODEL="${7:-${DEFAULT_MODEL:-claude-sonnet-4-6}}"
WORKER_DIR="${WORK_DIR}/logs/workers/${TASK_ID}"
...
TIMEOUT_EOF
  ...
  if [[ $ITERATION -gt 1 ]]; then
    ...
    if [[ -s "$PREV_OUTPUT" ]]; then
      ...
      PREV_CONTEXT="
## Previous iteration context (iteration ${PREV_ITER})
The previous attempt did not succeed. Last output:
\`\`\`
${PREV_TAIL}
\`\`\`
Analyze what went wrong and try a different approach. If blocked — write FAILURE to memory/learnings.md and set status: blocked in result.md.
"
    fi
  fi
...
${PROMPT}
${PREV_CONTEXT}
IMPORTANT: when the task is complete, write the result to ${RESULT_FILE} and exit.
Result format:
---
...
Detailed result..."
  ...
  if [ $GIT_EXIT -ne 0 ]; then
    echo "[$(date)] [worker-${TASK_ID}] git pull failed (exit $GIT_EXIT): $GIT_PULL_OUT" >> "${WORK_DIR}/memory/worker-errors.log"
    # Check for merge conflict markers
    if echo "$GIT_PULL_OUT" | grep -q -E "CONFLICT|conflict"; then
      CONFLICT_MSG="[GIT CONFLICT] Merge conflict in worker-${TASK_ID}. Details: memory/worker-errors.log. Manual resolution required."
      echo "[$(date)] $CONFLICT_MSG" >> "${WORK_DIR}/memory/worker-errors.log"
...
" 2>/dev/null || true)
  ...
  # NOT captured in variable — output flows in real-time
  cd "$WORK_DIR"

  # Build agent-specific --add-dir flags
  AGENT_DIR_FLAG=""
  if [[ -n "$AGENT_TYPE" ]] && [[ -d "$WORK_DIR/agents/$AGENT_TYPE" ]]; then
    AGENT_DIR_FLAG="--add-dir $WORK_DIR/agents/$AGENT_TYPE"
  fi
...
tmux new-session -d -s "$SESSION_NAME" -c "$WORK_DIR" \
  "bash ${WORKER_DIR}/run.sh ${TASK_ID} ${MAX_ITERATIONS} ${TIMEOUT_MIN} ${WORK_DIR} ${CLAUDE} ${AGENT_TYPE} ${MODEL} 2>&1 | tee ${WORKER_DIR}/output.log"
