...
when_to_use: Сохранить историю конкретного канала/чата как структурированный ресурс; разовый глубокий экспорт перед анализом (>50 сообщений); архивирование канала для ресерча; пользователь дал URL/username канала и попросил "экспортировать" или "сохранить".
allowed-tools: Read, Edit, Write, Bash, Grep
---

# Telegram Export — Процедура AgentOS

> Требует: `tdl` (MTProto-клиент). Установка: https://github.com/iyear/tdl. Если не настроен — пропусти этот скилл.
...
По умолчанию — аккаунт `default`. Если канал/чат доступен только с другого — использовать `tdl -n <namespace> ...`.
...
Без этой пометки артефакт не сохранять.

## Входные данные

- **URL или username** канала/чата (обязательно) — например: `@some_channel`, `t.me/communitysprints`, числовой ID
- **Количество сообщений** (default: 500)
- **Категория** (опционально) — если не указана, определяй автоматически
- **Namespace** (опционально) — `default` по умолчанию
...
Сначала реестр `${CLAUDE_PROJECT_DIR}/memory/telegram-chats.md` — там уже есть ID известных чатов.
...
```bash
ls "${CLAUDE_PROJECT_DIR}/resources/{category}/{slug}/posts/" 2>/dev/null | tail -1
```
...
```bash
python3 "${CLAUDE_PROJECT_DIR}/scripts/slim-tdl-export.py" /tmp/tg-export-raw.json /tmp/tg-export.json
```
...
Если чата не было в `${CLAUDE_PROJECT_DIR}/memory/telegram-chats.md` — добавить строку в соответствующую секцию.
