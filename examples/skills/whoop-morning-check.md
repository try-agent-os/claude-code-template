---
title: WHOOP Morning Check
summary: Получает данные здоровья из WHOOP MCP (recovery, HRV, RHR, sleep), обновляет memory/health.md и отправляет алерт оператору если recovery < 33%.
read_when: Каждое утро после 07:00 по расписанию (24ч интервал); задача типа whoop, health, recovery, утренний чек.
---

# Скилл: whoop-morning-check

Получает данные здоровья из WHOOP и записывает в `memory/health.md`.
При recovery < 33% отправляет алерт оператору.

## Когда запускать

- Каждое утро (после 07:00)
- Частота: 24ч (см. `memory/schedule.md`)

## Требования

- WHOOP MCP сервер запущен на `localhost:3850`
...
Перезаписать секцию `## Последние данные` в `memory/health.md`:
