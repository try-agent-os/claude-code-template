...
description: При новом или неполном контакте собирает данные из всех коннекторов (Telegram, LinkedIn, Gmail, Fireflies, Calendar, Web). Запускается автоматически при создании контакта или вручную.
type: procedure
---
...
### 1. Telegram (опционально, если установлен tdl)

Если у контакта есть username и в системе настроен `tdl` (https://github.com/iyear/tdl):
...
Поиск через WebSearch: `"{имя} {фамилия}" LinkedIn {компания}`
...
### 3. Gmail (опционально, если подключен Gmail MCP)
...
- Максимум 3 enrichment за один dispatcher цикл (rate limiting)
- Если коннектор не дает результатов — пропускать, не фейлиться
- НЕ обогащать внутренних контактов команды и спамеров
- Slug файла = имя-фамилия (lowercase, дефис): `john-smith.md`
