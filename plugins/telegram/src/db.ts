...
/**
 * Seed admin user IDs as 'allowed' in the users table. Idempotent — existing
 * rows are upgraded to 'allowed' if they were 'pending' or 'denied'. Called
 * once at startup from env (TELEGRAM_ADMIN_USER_IDS, comma-separated) so that
 * a fresh install grants the wizard-detected admins access without a manual
 * `/approve` round-trip.
 */
