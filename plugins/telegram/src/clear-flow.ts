...
// tmux session via scripts/operator-clear-inject.sh, which types `/clear` + Enter
// into the claude TUI (exactly how start.sh types `/boot` on boot). Claude runs
// its own in-place context-clear — NO process restart, MCP connections stay alive,
// so there is no anti-replay / suicide-restart concern.
//
...
const INJECT_SCRIPT =
  process.env.OPERATOR_CLEAR_INJECT_SCRIPT ||
  '/opt/agent-os/claude/scripts/operator-clear-inject.sh';
...
// server ($HOME/.tmux/tmux-<uid>/default) the same way start.sh created it.
export async function handleClear(): Promise<ClearResult> {
  try {
    const { stdout, stderr } = await execFileP(INJECT_SCRIPT, [], {
      timeout: 10_000,
      env: {
        ...process.env,
